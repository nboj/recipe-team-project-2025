...
export default function Home() {
  ...
  const trendingRecipe = useMemo(() => {
    ...
    const index =
      ((trendingIndex % recipes.length) + recipes.length) % recipes.length;
    return recipes[index];
  }, [recipes, hasRecipes, trendingIndex]);

  const [newestRecipe, setNewestRecipe] = useState<null | Recipe>(null);
  ...
  useEffect(() => {
    (async () => {
      let res = await fetchRecipes({sortBy: "created_at", descending: true, limit: 1})
      console.log(res)
      if (res) {
        setNewestRecipe(res[0])
      }
    })();
  }, [])
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-800 text-slate-100">
        ...
        <section className="border-b border-slate-800/60 bg-slate-900/40">
          <div className="mx-auto max-w-6xl px-4 py-10">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold tracking-wide">Trending</h3>
            ...
            <div className="grid gap-4 rounded-2xl border border-slate-800/60 bg-slate-950/60 p-4 md:grid-cols-[2fr,3fr]">
              ...
              <div className="flex flex-col justify-center">
                ...
                <p className="text-xs text-slate-500">
                  Cycling through {recipes.length} total recipes. Use the arrows
                  to explore.
                </p>
              </div>
            </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12">
        ...
        {/* status + list */}
        {loading && (
          <p className="mb-4 text-sm text-slate-300">Loading recipes…</p>
        )}
      ...
      <footer className="border-t border-slate-800/60">
        <div className="mx-auto max-w-6xl px-4 py-10 text-sm text-slate-400">
          {new Date().getFullYear()} Recipe Forge
        </div>
      </footer>
    </main>
  );
