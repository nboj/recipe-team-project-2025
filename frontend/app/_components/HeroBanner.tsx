          ...
          {slides.map((r) => (
            <div key={r.id} className="relative min-w-full">
              <div className="relative h-[60vh] w-full">
                <Image src={r.image} alt={r.title} fill priority className="object-cover" unoptimized />
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent" />
              </div>
              <div className="absolute bottom-10 left-8 right-8 max-w-2xl text-white">
                <h1 className="text-3xl md:text-5xl font-bold drop-shadow">{r.title}</h1>
                {r.summary && (
                  <p className="mt-3 text-sm md:text-base line-clamp-3 md:line-clamp-4">{r.summary}</p>
                )}
                <div className="mt-5 flex gap-3">
                  <button className="bg-white text-black rounded-lg px-4 py-2 font-medium">View Recipe</button>
